...
router.patch('/:videoId', (req, res, next) => {
  ...
  async.eachOfSeries(
    req.body.subtitles,
    (subtitle, subtitleIndex, cb) => {
      let update_dict = { $set: {} };
      update_dict['$set']['subtitles.' + subtitleIndex] = subtitle;
      ...
      const index = req.body.subtitles.indexOf(subtitle);
          ...
          Word.findOne(
            ...
            function(err, wordFound) {
              if (wordFound) {
                ...
                let update_word = { $set: {} };
                update_word['$set'][
                  'subtitles.' + index + '.words.' + wordIndex
                ] = wordFound;
                Video.update(
                  {
                    _id: videoId
